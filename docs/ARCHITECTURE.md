...
Наполнение каждого следующего уровня — не фоновый воркер: `sync_events` и статус `WorkOrder` пересчитываются в начале соответствующего `GET`-запроса, а прогнозы (`sync_risks` через ML Prediction Port и риски по объектам через `/risk_map`) считаются при старте приложения. Всё идемпотентно через high-water-mark (см. `docs/DATA_PROCESSING.md`). Единственный настоящий фоновый процесс — replay-движок СМВУ, потому что именно он должен выглядеть как непрерывный поток телеметрии.
